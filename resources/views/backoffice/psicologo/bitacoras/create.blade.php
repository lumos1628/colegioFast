<x-psicologo-layout :psicologo="$psicologo" :alumnos-atendidos="$alumnosAtendidos">
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <x-breadcrumb :items="[['label' => 'Bitácora', 'url' => route('psicologo.bitacoras.index')], ['label' => 'Nueva bitácora']]" />

        <h1 class="text-2xl font-bold text-gray-900 mb-6">Nueva Bitácora Psicológica</h1>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('psicologo.bitacoras.store') }}" method="POST">
                @csrf
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="alumno_id" required class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm @error('alumno_id') border-red-300 @enderror">
                            ...
                            @foreach($alumnos as $alumno)
                                <option value="{{ $alumno->id }}" {{ old('alumno_id') == $alumno->id ? 'selected' : '' }}>{{ $alumno->nombres }} {{ $alumno->apellido_paterno }} {{ $alumno->apellido_materno }} — {{ $alumno->grado }}°{{ $alumno->seccion }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="observaciones" rows="8" required
                                  class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm @error('observaciones') border-red-300 @enderror"
                                  placeholder="Describa las observaciones de la sesión psicológica...">{{ old('observaciones') }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700">Registrar bitácora</button>
                </div>
            </form>
        </div>
    </div>
