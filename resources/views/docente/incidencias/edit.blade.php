<x-docente-layout :docente="$docente" :cursos-por-dia="$cursosPorDia">
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <x-card>
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Editar Incidencia de Conducta</h1>
                <p class="mt-1 text-sm text-gray-500">
                    {{ $alumno->nombres }} {{ $alumno->apellido_paterno }} {{ $alumno->apellido_materno }}
                </p>
            </div>

            <form action="{{ route('docente.cursos.alumnos.incidencias.update', [$asignacion, $alumno, $incidencia]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all incidencia-option {{ old('tipo', $incidencia->tipo->value) === 'falta_leve' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <input type="radio" name="tipo" value="falta_leve"
                                       class="sr-only incidencia-radio"
                                       {{ old('tipo', $incidencia->tipo->value) === 'falta_leve' ? 'checked' : '' }}>
                            ...
                            <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all incidencia-option {{ old('tipo', $incidencia->tipo->value) === 'falta_grave' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <input type="radio" name="tipo" value="falta_grave"
                                       class="sr-only incidencia-radio"
                                       {{ old('tipo', $incidencia->tipo->value) === 'falta_grave' ? 'checked' : '' }}>
                                <div class="flex items-center">
                            ...
                            <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all incidencia-option {{ old('tipo', $incidencia->tipo->value) === 'merito' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <input type="radio" name="tipo" value="merito"
                                       class="sr-only incidencia-radio"
                                       {{ old('tipo', $incidencia->tipo->value) === 'merito' ? 'checked' : '' }}>
                                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" id="descripcion" rows="4" required
                                  ...
                                  placeholder="Describe la incidencia con detalle...">{{ old('descripcion', $incidencia->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $incidencia->fecha->format('Y-m-d')) }}" required
