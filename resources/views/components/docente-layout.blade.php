...
<head>
    ...
    <title>{{ config('app.name', 'colegioFast') }} - Portal Docente</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar" class="flex flex-col w-72 bg-white border-r border-gray-200 fixed md:relative h-full z-40 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out">
            {{-- Header del sidebar - Datos del docente --}}
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center space-x-3 mb-4">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white font-bold text-lg shrink-0">
                        {{ substr($docente->user->name, 0, 1) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-900 truncate">{{ $docente->user->name }}</h3>
                        <p class="text-sm text-gray-500 truncate">{{ $docente->especialidad }}</p>
                    </div>
                </div>
                <div class="space-y-2 text-sm text-gray-600">
                    <div class="flex items-center">
                        ...
                        <span class="truncate">{{ $docente->user->email }}</span>
                    </div>
                    @if($docente->telefono)
                        <div class="flex items-center">
                            <svg class="w-4 h-4 mr-2 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
